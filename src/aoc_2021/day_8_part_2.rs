...
#[allow(dead_code)]
fn day_8_part_2(input: &str) -> i64 {
    let mut sum: i64 = 0;

    let lines: Vec<&str> = input.split('\n').collect();

    for line in lines {
        let mut segment_mappings: Vec::<Vec<char>> = vec![vec![]; 7];

        let raw_data: Vec<&str> = line.split('|').collect();

        let mut signal_patterns: Vec<&str> = raw_data[0].split_whitespace().collect();

        if let Some(pattern_index) = signal_patterns.iter().position(|sig_pattern| sig_pattern.len() == 2) {
            let pattern = signal_patterns[pattern_index];

            segment_mappings[2] = pattern.chars().collect();
            segment_mappings[5] = pattern.chars().collect();

            signal_patterns.remove(pattern_index);
        }

        if let Some(pattern_index) = signal_patterns.iter().position(|sig_pattern| sig_pattern.len() == 3) {
            let pattern: Vec<char> = signal_patterns[pattern_index].chars().filter(|&ch| segment_mappings[2].iter().find(|&&mapped_ch| mapped_ch == ch) == None).collect();

            segment_mappings[0] = pattern.clone();

            signal_patterns.remove(pattern_index);
        }

        if let Some(pattern_index) = signal_patterns.iter().position(|sig_pattern| sig_pattern.len() == 4) {
            let pattern: Vec<char> = signal_patterns[pattern_index].chars().filter(|&ch| segment_mappings[2].iter().find(|&&mapped_ch| mapped_ch == ch) == None).collect();

            segment_mappings[1] = pattern.clone();
            segment_mappings[3] = pattern.clone();
...

        {
            ...
            pattern.extend(segment_mappings[2].clone());
            pattern.extend(segment_mappings[0].clone());
            pattern.extend(segment_mappings[1].clone());

            let inverse: Vec<char> = vec!['a', 'b', 'c', 'd', 'e', 'f', 'g'].into_iter().filter(|ch| pattern.iter().find(|&pattern_ch| pattern_ch == ch) == None).collect();

            segment_mappings[4] = inverse.clone();
            segment_mappings[6] = inverse.clone();
        }

        {
            let mut pattern = Vec::<char>::new();
            while let Some(signal_index) = signal_patterns.iter().position(|sig_pattern| sig_pattern.len() == 5) {
                pattern.extend(signal_patterns[signal_index].chars().collect::<Vec<char>>());
                signal_patterns.remove(signal_index);
            }
            for ch in vec!['a', 'b', 'c', 'd', 'e', 'f', 'g'] {
                if pattern.iter().filter(|&&pattern_ch| pattern_ch == ch).count() > 1 {
                    pattern.retain(|&pattern_ch| pattern_ch != ch);
                }
            }
            segment_mappings[1].retain(|mapping_ch| pattern.iter().find(|&pattern_ch| pattern_ch == mapping_ch) != None);
            segment_mappings[3].retain(|mapping_ch| pattern.iter().find(|&pattern_ch| pattern_ch == mapping_ch) == None);
            segment_mappings[4].retain(|mapping_ch| pattern.iter().find(|&pattern_ch| pattern_ch == mapping_ch) != None);
            segment_mappings[6].retain(|mapping_ch| pattern.iter().find(|&pattern_ch| pattern_ch == mapping_ch) == None);
        }

        {
            while let Some(signal_index) = signal_patterns.iter().position(|sig_pattern| sig_pattern.len() == 6) {
                let mut unique_ch: char = ' ';
                let mut unique_char_found = false;
                for mapped_ch in segment_mappings[2].iter() {
                    if signal_patterns[signal_index].chars().find(|&pattern_ch| pattern_ch == *mapped_ch) == None {
                        unique_ch = *mapped_ch;
                ...
                if unique_char_found {
                    segment_mappings[2] = vec![unique_ch];
                    segment_mappings[5].retain(|&ch| ch != unique_ch);
                    break;
                }
            }
        }

        let segment_number_descs = HashMap::from([
            (0, vec![0, 1, 2, 4, 5, 6]),
            (1, vec![2, 5]),
            (2, vec![0, 2, 3, 4, 6]),
            (3, vec![0, 2, 3, 5, 6]),
        ...
        for segment_number_desc in segment_number_descs {
            let mut num_string = Vec::<char>::new();
            for n in segment_number_desc.1 {
                num_string.push(segment_mappings[n][0]);
            }
            num_string.sort();
            inverse_mapping.insert(String::from_iter(num_string), segment_number_desc.0);
        }

        let output_digits: Vec<&str> = raw_data[1].split_whitespace().collect();

        let mut ans: i64 = 0;
        for output_digit in output_digits {
            let mut num_string: Vec<char> = output_digit.chars().collect();
            num_string.sort();
            let num = inverse_mapping.get(&String::from_iter(num_string)).unwrap();
            ans *= 10;
            ans += num;
        }
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce"#;
    ...
    fn test_day_8_part_2() {
        let input =
r#"gea agfcbe egcbfd aecb cegbf cafgde fgeba ea gabdf begfdca | bcfeg ea fbdga fbgcdae
