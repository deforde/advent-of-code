use crate::aoc_2021::day_18_part_1::explode_snail;
use crate::aoc_2021::day_18_part_1::split_snail;
use crate::aoc_2021::day_18_part_1::get_snail_magnitude;

#[allow(dead_code)]
fn day_18_part_2(input: &str) -> i64 {
    let mut max_mag = 0;
    let inputs = input.split('\n').collect::<Vec<_>>();

    for i in 0..inputs.len() {
        for j in 0..inputs.len() {
            if i != j {
                let mut sum = String::new();

                sum.push('[');
                sum.push_str(&inputs[i]);
                sum.push(',');
                sum.push_str(&inputs[j]);
                sum.push(']');

                while explode_snail(&mut sum) || split_snail(&mut sum) {
                }

                let ans = get_snail_magnitude(&mut sum);

                max_mag = i64::max(max_mag, ans);
            }
        }
    }

    return max_mag;
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn example_day_18_part_2() {
        let input =
r#"[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]
[[[5,[2,8]],4],[5,[[9,9],0]]]
[6,[[[6,2],[5,6]],[[7,6],[4,7]]]]
[[[6,[0,7]],[0,9]],[4,[9,[9,0]]]]
[[[7,[6,4]],[3,[1,3]]],[[[5,5],1],9]]
[[6,[[7,3],[3,2]]],[[[3,8],[5,7]],4]]
[[[[5,4],[7,7]],8],[[8,3],8]]
[[9,3],[[9,9],[6,[4,9]]]]
[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]
[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]"#;

        let ans = day_18_part_2(&input);

        assert_eq!(ans, 3993);
    }

    #[test]
    fn test_day_18_part_2() {
        let input =
r#"[[7,[1,5]],[[5,7],[[0,8],2]]]
[[[[7,3],[2,2]],3],[[[7,1],[9,1]],2]]
[[[3,[0,2]],[5,2]],8]
[[[[1,5],8],[[0,5],[0,0]]],2]
[[[[9,9],1],[8,[3,2]]],[0,8]]
[[[[0,8],6],6],[[1,5],[[5,5],[4,6]]]]
[[3,[[9,0],[7,6]]],[7,4]]
[6,[[8,[9,7]],[[1,1],[2,6]]]]
[[[[2,2],6],[2,6]],[[8,[3,2]],[2,[6,5]]]]
[9,[1,[1,[4,6]]]]
[8,[[5,[7,7]],[2,2]]]
[[[[3,2],[4,9]],[6,8]],[[[7,9],9],[7,5]]]
[[[[0,4],[9,6]],0],[4,[6,[5,1]]]]
[[3,[4,5]],[1,[[2,8],4]]]
[[[9,[8,3]],[[0,0],2]],[[1,3],[[8,0],[5,3]]]]
[[[[2,4],4],[[5,8],4]],9]
[[1,[[0,8],[1,0]]],[1,[9,2]]]
[[[[7,3],5],[7,[3,1]]],[[3,1],[9,[2,0]]]]
[0,[[7,[9,3]],9]]
[[5,0],[[5,1],[2,3]]]
[[2,[[8,1],0]],[[0,[9,0]],[[3,4],[8,6]]]]
[[[7,5],8],[6,6]]
[[[2,4],[[4,7],[9,6]]],[6,[[3,5],0]]]
[7,[[3,2],[[9,3],9]]]
[[[[9,9],2],[[1,0],6]],[[[6,4],8],[[4,7],[5,6]]]]
[[[1,0],[[3,1],2]],[[[5,3],6],[2,[8,4]]]]
[[4,[[8,1],2]],[2,3]]
[[[[9,6],1],4],1]
[[5,[3,1]],[[[0,5],5],[[4,2],6]]]
[[6,[8,7]],[[[0,9],6],9]]
[[2,[[2,8],6]],[[[7,9],8],[6,[7,6]]]]
[[2,[[4,6],[7,5]]],[[[3,5],[6,4]],[[6,8],4]]]
[[[[1,8],[0,8]],[0,[5,0]]],[[[2,5],[0,6]],8]]
[[[[8,5],[2,3]],[[2,6],4]],[[0,9],1]]
[[2,[6,4]],8]
[[0,8],[[[1,6],[6,3]],[[2,2],8]]]
[0,[[[7,1],[4,7]],5]]
[[[7,[1,8]],[2,[1,3]]],9]
[[1,[9,1]],[[6,8],[6,[7,3]]]]
[[[[5,1],[6,9]],[0,5]],[0,[[6,5],3]]]
[[2,[[3,4],1]],[4,1]]
[[[[1,6],6],[[6,5],0]],[7,[0,4]]]
[[2,[[6,9],[9,4]]],3]
[[[1,3],[7,9]],3]
[1,[3,[[1,0],4]]]
[[[9,1],[4,[5,1]]],[[6,0],[1,2]]]
[[[3,[0,3]],9],[[6,[7,0]],[1,1]]]
[[2,[0,[1,5]]],[[0,[7,7]],[[4,9],[8,4]]]]
[[7,[[5,1],[6,4]]],[[2,1],9]]
[[5,6],[[8,6],[[9,1],[7,2]]]]
[[[[0,6],5],7],[[[6,7],9],1]]
[[4,[0,[6,9]]],[6,[[5,6],3]]]
[[[[0,0],5],[[1,3],0]],[[[4,1],6],7]]
[7,5]
[[[8,[9,5]],[0,1]],[3,[[0,1],9]]]
[[[[2,1],5],1],[[[8,1],[1,9]],1]]
[[[[0,9],[7,3]],[8,0]],[[4,[9,5]],7]]
[6,[[9,[4,7]],1]]
[[7,1],[[9,7],[7,[5,5]]]]
[[4,[6,[7,1]]],[[2,2],[[0,9],3]]]
[[[6,[5,4]],[9,4]],0]
[5,[[[7,8],5],7]]
[[[[1,8],[2,7]],[3,[4,4]]],[[[0,9],[4,5]],[[9,8],[0,6]]]]
[[[[0,2],6],2],[[[5,8],[1,3]],[[2,5],5]]]
[[3,[0,[2,3]]],[[[3,0],8],3]]
[[[[8,5],[7,2]],[8,7]],[[[8,1],[2,5]],5]]
[[[[6,0],[5,6]],[[7,8],[3,5]]],[[[8,3],9],[6,[5,8]]]]
[[7,0],[[[4,3],[4,3]],[[7,1],[8,9]]]]
[[[0,0],[7,0]],[3,[[3,6],1]]]
[[[6,6],1],[0,[1,[1,5]]]]
[[[1,[7,8]],[[7,7],[5,1]]],[0,6]]
[0,[[7,4],[7,7]]]
[[[4,[8,3]],[[2,9],6]],[2,[4,0]]]
[7,[8,8]]
[[[[1,4],1],3],[[3,2],3]]
[[7,[0,4]],[[[9,1],[4,4]],[4,8]]]
[4,[[1,8],9]]
[[[[5,7],[8,5]],[9,2]],[[7,5],7]]
[[[3,[3,2]],3],[8,[9,0]]]
[[2,0],9]
[[8,[[6,4],[2,2]]],[[[4,8],6],[2,[1,5]]]]
[[6,[[0,4],[3,0]]],[[2,5],[3,3]]]
[[[[6,8],[6,1]],[1,3]],[[[4,9],[0,8]],9]]
[[[1,[1,6]],[[2,8],5]],[[[8,1],9],6]]
[[[3,[9,6]],[1,[6,5]]],[[4,7],[[4,4],8]]]
[[[7,6],0],[[7,7],7]]
[[3,0],[1,[[2,1],9]]]
[[9,[[3,7],9]],[[[1,1],4],[0,[3,9]]]]
[[3,[1,[0,1]]],[[0,9],[4,[8,8]]]]
[[[8,[3,2]],[9,5]],[[[1,2],[0,4]],6]]
[[5,4],[[2,4],[0,[7,2]]]]
[[[[9,2],4],[[9,5],[8,8]]],[[7,[7,0]],3]]
[[[[8,2],[1,1]],[[9,3],3]],[[9,[5,2]],[[1,4],[0,6]]]]
[4,[[[2,7],6],[[0,8],0]]]
[[[4,4],3],[0,8]]
[[3,[3,[8,6]]],[5,[[0,6],3]]]
[[[2,[0,9]],[[5,0],[7,3]]],[[8,1],[5,2]]]
[[7,[[3,2],[6,8]]],[2,[[4,7],[3,1]]]]
[[[[6,3],6],4],[[[7,8],[5,1]],[[3,0],5]]]
[[3,[9,5]],[6,9]]"#;

        let ans = day_18_part_2(&input);

        assert_eq!(ans, 4500);
    }
}
