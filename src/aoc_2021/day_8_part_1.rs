#[allow(dead_code)]
fn day_8_part_1(input: &str) -> usize {
    let lines: Vec<&str> = input.split('\n').collect();

    let unique_digit_lens = vec![2, 4, 3, 7];

    let mut unique_output_digit_count: usize = 0;
    ...
    for line in lines {
        ...
        for output_digit in output_digits {
            if let Some(_) = unique_digit_lens.iter().find(|x| **x == output_digit.len()) {
                unique_output_digit_count += 1;
            }
        }
    }

    return unique_output_digit_count;
}
