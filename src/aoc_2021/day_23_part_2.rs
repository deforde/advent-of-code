fn parse_input(input: &str) -> Vec::<(char, char, char, char)> {
    let lines = input.split('\n').collect::<Vec<_>>();

    let mut rooms: Vec::<(char, char, char, char)> = vec![('.', '.', '.', '.'); 4];
    ...
    for (idx, ch) in lines[4].chars().filter(|c| c.is_alphabetic()).enumerate() {
        rooms[idx].2 = ch;
    }
    for (idx, ch) in lines[5].chars().filter(|c| c.is_alphabetic()).enumerate() {
        rooms[idx].3 = ch;
    }
...
fn is_solved(rooms: &Vec::<(char, char, char, char)>) -> bool {
...
fn iterate_sim(rooms: &Vec::<(char, char, char, char)>, hallway: &Vec::<char>, current_cost: i64, min_cost: &mut i64) {
    // println!("rooms = {:?}", rooms);
    // println!("hallway = {:?}\n", hallway);
    ...
    for (idx, &pod) in hallway.iter().enumerate() {
        if pod != '.' {
            ...
            if rooms[room_idx].3 == '.' {
                ...
                new_hallway[idx] = '.';
                new_rooms[room_idx].3 = pod;

                let new_cost = current_cost + get_cost_of_move(pod, room_idx, 3, idx);
                iterate_sim(&new_rooms, &new_hallway, new_cost, min_cost);
            }
            else if rooms[room_idx].2 == '.' && rooms[room_idx].3 == pod {
                let mut new_rooms = rooms.clone();
                let mut new_hallway = hallway.clone();

                new_hallway[idx] = '.';
                new_rooms[room_idx].2 = pod;

                // println!("Moving \'{}\' from hallway {} to room {} pos {}", pod, idx, room_idx, 0);

                let new_cost = current_cost + get_cost_of_move(pod, room_idx, 2, idx);
                iterate_sim(&new_rooms, &new_hallway, new_cost, min_cost);
            }
            else if rooms[room_idx].1 == '.' && rooms[room_idx].3 == pod && rooms[room_idx].2 == pod {
                let mut new_rooms = rooms.clone();
                let mut new_hallway = hallway.clone();
            ...
            else if rooms[room_idx].0 == '.' && rooms[room_idx].3 == pod && rooms[room_idx].2 == pod && rooms[room_idx].1 == pod {
                let mut new_rooms = rooms.clone();
                let mut new_hallway = hallway.clone();
    ...
    for (idx, (pod0, pod1, pod2, pod3)) in rooms.iter().enumerate() {
        if *pod0 != '.' && (
                idx != get_room_index_for_pod_type(*pod0) || (
                    idx == get_room_index_for_pod_type(*pod0) && (
                        (*pod1 != '.' && *pod0 != *pod1) || (*pod2 != '.' && *pod0 != *pod2) || (*pod3 != '.' && *pod0 != *pod3)
                    )
                )
            ) 
        {
        ...
        if *pod0 == '.' && *pod1 != '.' && (
                idx != get_room_index_for_pod_type(*pod1) || (
                    idx == get_room_index_for_pod_type(*pod1) && (
                        (*pod2 != '.' && *pod1 != *pod2) || (*pod3 != '.' && *pod1 != *pod3)
                    )
                )
            )
        {
        ...
        if *pod0 == '.' && *pod1 == '.' && *pod2 != '.' && (
                idx != get_room_index_for_pod_type(*pod2) || (
                    idx == get_room_index_for_pod_type(*pod2) && *pod3 != '.' && *pod2 != *pod3
                )
            )
        {
            let door_idx = get_door_index_for_room_index(idx);
            for hallway_idx in (0..door_idx).rev() {
                if is_valid_hallway_index(hallway_idx) {
                    ...
                    new_hallway[hallway_idx] = *pod2;
                    new_rooms[idx].2 = '.';

                    // println!("Moving \'{}\' from room {} pos {} to hallway {}", *pod0, idx, 0, hallway_idx);

                    let new_cost = current_cost + get_cost_of_move(*pod2, idx, 2, hallway_idx);
                    iterate_sim(&new_rooms, &new_hallway, new_cost, min_cost);
                }
            }
            for hallway_idx in door_idx + 1..hallway.len() {
        ...
        if *pod0 == '.' && *pod1 == '.' && *pod2 == '.' && *pod3 != '.' && idx != get_room_index_for_pod_type(*pod3) {
            let door_idx = get_door_index_for_room_index(idx);
            for hallway_idx in (0..door_idx).rev() {
                if is_valid_hallway_index(hallway_idx) {
                    if hallway[hallway_idx] != '.' {
                    ...
                    new_hallway[hallway_idx] = *pod3;
                    new_rooms[idx].3 = '.';

                    // println!("Moving \'{}\' from room {} pos {} to hallway {}", *pod1, idx, 1, hallway_idx);

                    let new_cost = current_cost + get_cost_of_move(*pod3, idx, 3, hallway_idx);
                    iterate_sim(&new_rooms, &new_hallway, new_cost, min_cost);
                }
            }
            for hallway_idx in door_idx + 1..hallway.len() {
...
#[allow(dead_code)]
fn day_23_part_2(input: &str) -> i64 {
